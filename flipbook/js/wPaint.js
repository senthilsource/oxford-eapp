(function ($) {
  ...
  Paint.prototype = {
    generate: function () {
      ...
      function createCanvas(name) {
        ...
        _this[ctxName].globalAlpha = 0;
        _this[ctxName].fillRect(50, 20, 40, 40);
        _this['$' + canvasName] = $(_this[canvasName]);
        
        _this['$' + canvasName]
        .attr('class', 'wPaint-canvas' + (name ? '-' + name : ''))
        .attr('width', _this.width + 'px')
        .attr('height', _this.height+100 + 'px')        
    ...
    _displayStatus: function (msg) {
      ...
      if (!this.$status) {
        this.$status = $('<div class="alert alert-success " ></div>');
      ...
      this.$status.fadeIn(1500, function () {
        _this.displayStatusTimer = setTimeout(function () { _this.$status.fadeOut(1500); }, 1500);
      });
    },
  ...
  Menu.prototype = {
    generate: function () {
      this.$menu = $('<div class="wPaint-menu" style="position: relative;left: -50%;top: 6%;width: 54px;"></div>');
    ...
    _createIconBase: function (item) {
      ...
      if(item.title==="Rectangle"){
        item.img="/ic_4.png";
      }
      if(item.title==="Clear"){
        item.img="/clear.png";
      }
      if(item.title=="Pen Highlighter"){
        item.img="ic_3.png";
      }
    ...
    _createSelectBox: function ($icon) {
      ...
      function iconClick() {
        $selectHolder.toggle();
      //   $("#wpaint").attr("style", "position:absolute; z-index: 9999; top:9%; left:37%;width:800px; height:400px; margin:70px auto 20px auto;");
      //   $("#wPaint").show();
      //   $('#wPaint').wPaint({
      //   menuOffsetLeft: -100,
      //   menuOffsetTop: -40,
